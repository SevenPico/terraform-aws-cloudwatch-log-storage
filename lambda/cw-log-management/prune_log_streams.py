...
def _prune_log_streams(group: dict, purge_non_empty: bool = False, dry_run: bool = False):
...
def prune_log_streams(log_group_name_prefix: str = None, purge_non_empty: bool = False, dry_run: bool = False):
    ...
    for response in cw_logs.get_paginator("describe_log_groups").paginate(**kwargs):
        for group in response["logGroups"]:
            _prune_log_streams(group, purge_non_empty, dry_run)
...
def handle(request, context):
    ...
    if "days" in request:
        set_log_retention.set_log_retention(days, dry_run)

    log_group_name_prefix = request.get("log_group_name_prefix")
    if log_group_name_prefix:
        prune_log_streams(log_group_name_prefix, purge_non_empty, dry_run)
    else:
